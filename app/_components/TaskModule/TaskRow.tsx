...
export default function TaskRow({
  ...
  return (
    <div
      className={`flex flex-shrink-0 w-full items-center gap-5 p-2 mb-2 cursor-pointer transition-all duration-300 rounded-md ${activeClass} text-primary dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-gray-700`}
      onClick={selectTask}>
      ...
      <div className="ml-auto flex justify-end">
        {onPin && (
          <Button onClick={pinTask} ghost icon={<StarOutlined className={`text-sm !w-4 !h-4 ${pinnedClass}`} />} />
        )}
      </div>
    </div>
  );
