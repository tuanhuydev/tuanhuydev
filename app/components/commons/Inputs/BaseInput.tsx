...
export const BaseInput = React.forwardRef<HTMLInputElement, InputProps>((props, ref) => {
  const { className = "", startAdornment, disabled = false, ...restProps } = props;
  const { getRootProps, getInputProps } = useInput(restProps);
  const { value, ...inputProps } = getInputProps();

  const defaultClasses =
    "w-full rounded-md flex items-center border-solid border outline-1 transition-all border-slate-300 bg-white dark:bg-slate-800 dark:border-slate-600 dark:text-slate-50";
  return (
    <div
      {...getRootProps()}
      ...
      <input
        ...
        value={value}
        ref={ref}
        className="w-full font-normal outline-none border-none bg-transparent placeholder:text-slate-400 disabled:bg-slate-200 disabled:cursor-not-allowed py-2 px-3"
      />
    </div>
  );
});
