    ...
    <section id="experience" className="py-10 md:py-24">
      ...
      <div className="cards flex flex-wrap gap-8">
        {MOCK_PROJECTS.map((project: ObjectType) => (
          <div
            className={`${styles.card} items-center relative p-3 h-96 w-full hover:w-full md:w-80 md:hover:w-96 max-w-full border-2 border-slate-200 dark:border-slate-700 rounded-md`}
            key={project.title}
          >
            <div className="flex min-w-0">
              <div
                className={`${styles.image} mr-3 shrink-0 relative flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-50`}
              >
                {project?.image && (
                  <Image
                    src={project.image}
            ...
            <div className="relative">
              ...
              <p
                className={`${styles.description} truncate-paragraph mt-5 text-slate-400`}
              >
                {project.description}
              </p>
            </div>
